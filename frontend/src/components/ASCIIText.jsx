// Component ported and slightly adapted from
...
void main() {
    ...
    /* Single sample: no RGB offset (chromatic aberration was causing red bleed that followed the wave) */
    gl_FragColor = tex;
}
`;
...
class AsciiFilter {
  ...
  hue() {
    /* Animate hue in cool range only (cyan–blue, ~180–260deg) to avoid red/magenta bleed */
    const t = typeof performance !== 'undefined' ? performance.now() * 0.0006 : 0;
    const deg = 200 + 55 * Math.sin(t);
    this.domElement.style.filter = `hue-rotate(${deg}deg)`;
  }
...
class CanvAscii {
  ...
  async init() {
    try {
      ...
      // ignore font loading failures and fall back
    }
